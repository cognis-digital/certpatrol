"""Native cognis-connect emit for certpatrol — forward findings to any platform.

Maps certpatrol's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    certpatrol ... --format json | certpatrol-emit --to stix
    certpatrol-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('domain', '') + ' ' + rec.get('message', '')

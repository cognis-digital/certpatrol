...
TOOL_VERSION = "0.1.0"
RULES = [{'id': 'CP-NEW-001', 'severity': 'high', 'weight': 2.5, 'title': 'UNEXPECTED_NEW_CERT', 'when': {'in_ct_log': True, 'in_expected_issuers': False}, 'remediation': 'Verify with infra team. If unauthorized, revoke via the issuing CA.'}, {'id': 'CP-EXP-001', 'severity': 'medium', 'weight': 2.0, 'title': 'CERT_EXPIRES_SOON', 'when': {'days_to_expiry': '^([0-9]|1[0-4])$'}, 'remediation': 'Renew immediately.'}]
        ...
        for rule in RULES:
            if _match(item, rule):
                result.add(Finding(
                    ...
                    location=str(target),
                    remediation=rule.get("remediation",""), category="cert-watch",
                    metadata=item,
                ))
